...
from Patch_data import sample_bg_patches_multifield, sample_bg_slices_at_indices_multifield
...
_SHUFFLED_MODES = ("shuffled", "permute", "epoch_shuffle")
_PERM_CACHE = {}


def _epoch_perm(depth, seed, step):
    """'shuffled' mode: a random permutation of the depth slices re-drawn every epoch,
    so each epoch visits every slice exactly once in random order (NeurLZ's own loop
    does the same with np.random.shuffle). The samplers receive the per-step seed
    seed0 + ep*10000 + step, so seed - step recovers an epoch-level seed. Cached: a
    33120-entry permutation per step would cost ~20% of a QMCPack step."""
    key = (int(depth), int(seed) - int(step))
    perm = _PERM_CACHE.get(key)
    if perm is None:
        _PERM_CACHE.clear()
        perm = np.random.default_rng(key[1] & 0xFFFFFFFF).permutation(int(depth))
        _PERM_CACHE[key] = perm
    return perm


def _shuffled_z(depth, n, step, seed):
    perm = _epoch_perm(depth, seed, step)
    return [int(perm[(int(step) * n + i) % depth]) for i in range(n)]
...
def _bg_full_slice_zlist(cfg, ep, step, depth, n, seed):
    ...
    if mode in _SHUFFLED_MODES:
        return _shuffled_z(depth, n, step, seed)
...
def _sample_bg_training_batch(
    ...
    *,
    ...
    if mode in _SHUFFLED_MODES:
        z = np.array(_shuffled_z(depth, n, step, seed), dtype=np.int64)
        return sample_bg_slices_at_indices_multifield(
            Xs_for_sampling,
            Xps,
            z,
...
def _sample_slice2d_gpu(Xs_gpu, Xps_gpu, cfg, ep, step, *, seed):
    ...
    if mode in ("z_shard", "z_shards", "shard_parallel"):
        ...
        ys = [0] * n
        xs = [0] * n
    elif mode in _SHUFFLED_MODES:   # every slice once per epoch, random order (corner crop like sequential)
        zs = _shuffled_z(depth, n, step, seed)
        ys = [0] * n
        xs = [0] * n
