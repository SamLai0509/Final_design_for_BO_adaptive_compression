...
origaux + noaux pkls are located from Reproduce/logs/auxq_ablation.log stage
windows (mtime inside [start, exit]); pins are written to
Reproduce/results/AUXQ_LADDER_PINS.json.
    python plot_aux_quality.py -> figures/nyx_aux_quality_ladder.{pdf,png}
"""
...
wins = stage_windows()
pins = json.load(open(os.path.join(R, "results", "PAPER_FINAL_CACHES.json")))
cA = json.load(open(os.path.join(R, "results", "CASCADE_A_CACHES.json")))
cB = json.load(open(os.path.join(R, "results", "CASCADE_B_CACHES.json")))
...
sel = {  # panel -> label -> pkl
  ...
  "Temp":   {"Original aux": new["origaux"]["nyx_t"], "Enhanced aux": cB["Temp"],   # single cascade DMD->T->BD: temperature is stage 2 (enh DMD only)
...
json.dump({"selection": sel, "windows": {f"{k[0]}:{k[1]}": v for k, v in wins.items()},
           "note": "origaux/noaux on RTX PRO 6000, paper protocol (shuffled, lr [1e-3,1e-2]); "
                   "enhanced = single cascade DMD->T->BD (repo pin CASCADE_B): baryon stage 3, temperature stage 2; decompressed = PAPER_FINAL pins"},
          open(os.path.join(R, "results", "AUXQ_LADDER_PINS.json"), "w"), indent=1)
...
print("Saved: figures/nyx_aux_quality_ladder.pdf")
for panel in sel:
